		...
		if (health.IsAlive()) {
			if (Visible_Targets.Count > 0) {
				...
				if (CompareTag("CC")) {
                    ...
                    bool rangeToShoot = distanceFromTarget < Shoot_Radius;
                    Debug.Log(rangeToShoot);

					if (shooter != null && health != null && health.IsAlive () && rangeToShoot) {
						shooter.SetShooting (true);
					} else if (!rangeToShoot) {
						Mouvement(alarmObj.transform);
                        /*AlarmControl alarm = GameObject.Find("Alarm").GetComponent<AlarmControl>();
                        alarm.SetAlertMode(true);*/
                    }
				}
			} else {
